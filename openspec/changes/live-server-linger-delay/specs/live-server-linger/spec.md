...
### Requirement: Paladin passes linger flag to Spektrum
The Paladin CLI SHALL accept `--live-linger` and forward it to the Spektrum argv when `--live` is active.

#### Scenario: Paladin forwards linger to spektrum
- **WHEN** `paladin ... --live --live-linger 10` is invoked
